...
var Slider = function () {
  function Slider(el) {
    ...
    // document.getElementById('slider-dots').addEventListener('click', this.onDotClick.bind(this));s
  ...
  Slider.prototype.positionImage = function positionImage(_ref5) {
    ...
    var maxImgOffset = 1;
    var currentImage = this.activeImg[0].querySelectorAll("img");

    for(var i=0;i<currentImage.length;i++){
      /*var ranNum = Number(Math.random());
      console.log(ranNum);*/
      if(i % 2 != 0){
         currentImage[i].style.setProperty('transform', '\n      translateX(' + maxImgOffset * -xCoeff  + 'em)\n      translateY(' + maxImgOffset * yCoeff  + 'em)\n    ');
      }
     }
  };
